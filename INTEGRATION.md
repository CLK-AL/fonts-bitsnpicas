...
| --- | --- | --- | --- |
| **fonts-bitsnpicas** *(host)* | `Glyph`, `BitmapGlyph`, `BitmapFont`, `ByteReader`/`ByteWriter`, **PSF/FNT/BDF/Hex importers+exporters**, `PlaydateImporter` (metadata+pixels→BitmapFont), **PUAA** (binary table + 4 text codecs), **TTF** (file envelope + 10 essential tables: head/name/post/cmap/os2/hhea/hmtx/maxp/loca/glyf — functional TTF round-trip); KMP 7-target; GraalVM native-image CLI; **ProGuard 7.9.0** (91 KB shrunk JAR + `verifyProguardedJar` passing 205 tests) | 287 | ~70 remaining TTF tables (UnknownTable fallback); ~15 remaining PUAA text codecs |
