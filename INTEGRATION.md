...
| --- | --- | --- | --- |
| **fonts-bitsnpicas** *(host)* | **Bitmap formats**: PSF/FNT/BDF/Hex import+export, Playdate import. **TTF**: 18 tables ported (head/name/post/cmap/os2/hhea/hmtx/maxp/loca/glyf + COLR/CPAL/SVG/CBLC/CBDT/kern/GPOS/GSUB). **PUAA**: binary table + 34 text codecs (full registry). **UiDriver**: Swing oracle + FontBox TTF outline renderer. KMP 7-target. GraalVM native-image. ProGuard 7.9.0. | 355 | ~50 remaining TTF tables (UnknownTable fallback); GPOS/GSUB lookup parsing; CBDT entry formats |
