...
public class BitmapFontGlyph extends FontGlyph {
	...
	public static BitmapFontGlyph compose(BitmapFontGlyph... glyphs) {
		...
		for (BitmapFontGlyph glyph : glyphs) {
			if (glyph == null || glyph.glyph == null) continue;
			...
			for (byte[] row : glyph.glyph) {
				// C5: null-check each row before dereferencing row.length.
				if (row == null) continue;
				if (glyph.x + row.length > x1) x1 = glyph.x + row.length;
			}
		}
		// C5: strict inequality so zero-width / zero-height composites are rejected.
		...
		for (BitmapFontGlyph glyph : glyphs) {
			...
			for (int y = 0; y < glyph.glyph.length; y++) {
				// C5: skip null rows rather than NPE.
				byte[] row = glyph.glyph[y];
				if (row == null) continue;
				for (int x = 0; x < row.length; x++) {
					if ((row[x] & 0xFF) > (g[by + y][bx + x] & 0xFF)) {
						g[by + y][bx + x] = row[x];
					}
				}
			}
